	...
	var HasMany = Relation.extend({
		...
		initialize: function( opts ) {
			...
			if ( _.isString( this.collectionType ) ) {
				this.collectionType = Relational.store.getObjectByName( this.collectionType );
			}
		...
		setKeyContents: function( keyContents ) {
			...
			if ( !this.keyContents && ( keyContents || keyContents === 0 ) ) { // since 0 can be a valid `id` as well
				...
				_.each( this.keyContents, function( item ) {
					var itemId = Relational.store.resolveIdForItem( this.relatedModel, item );
					if ( itemId || itemId === 0 ) {
		...
		handleAddition: function( model, coll, options ) {
			...
			var dit = this;
			!options.silent && Relational.eventQueue.add( function() {

...
import { default as Relational } from './Relational.js';
	...
	var RelationalModel = Backbone.Model.extend({
		...
		constructor: function( attributes, options ) {
			...
			Relational.store.processOrphanRelations();
			Relational.store.listenTo( this, 'relational:unregister', Relational.store.unregister );

			this._queue = new Backbone.BlockingQueue();
			this._queue.block();
			Relational.eventQueue.block();
		...
		trigger: function( eventName ) {
			if ( eventName.length > 5 && eventName.indexOf( 'change' ) === 0 ) {
				...
				if ( !Relational.eventQueue.isBlocked() ) {
					// If we're not in a more complicated nested scenario, fire the change event right away
					Backbone.Model.prototype.trigger.apply( dit, args );
				}
				else {
					Relational.eventQueue.add( function() {
				...
				Relational.store.unregister( this );
			}
			else {
				Backbone.Model.prototype.trigger.apply( this, arguments );
			}
		...
		initializeRelations: function( options ) {
			...
			_.each( this.relations || [], function( rel ) {
				Relational.store.initializeRelation( this, rel, options );
			}, this );
		...
		set: function( key, value, options ) {
			...
			try {
				...
				Relational.store.checkId( this, newId );
				...
				if ( !this._isInitialized && !this.isLocked() ) {
					...
					if ( newId || newId === 0 ) {
						Relational.store.register( this );
					}
	...
	},
	{
		...
		setup: function( superModel ) {
			...
			if ( this.prototype.hasOwnProperty( 'subModelTypes' ) ) {
				Relational.store.addSubModels( this.prototype.subModelTypes, this );
			}
			...
			_.each( this.prototype.relations || [], function( rel ) {
				...
				if ( rel.reverseRelation && rel.model === this ) {
					...
					if ( _.isString( rel.relatedModel ) ) {
						 ...
						 */
						var relatedModel = Relational.store.getObjectByName( rel.relatedModel );
						preInitialize = relatedModel && ( relatedModel.prototype instanceof Backbone.RelationalModel );
					}

					if ( preInitialize ) {
						Relational.store.initializeRelation( null, rel );
					}
					else if ( _.isString( rel.relatedModel ) ) {
						Relational.store.addOrphanRelation( rel );
					}
				}
			}, this );
		...
		initializeModelHierarchy: function() {
			...
			if ( this.prototype.subModelTypes ) {
				...
				_.each( unresolvedSubModels, function( subModelTypeName ) {
					var subModelType = Relational.store.getObjectByName( subModelTypeName );
		...
		inheritRelations: function() {
			...
			Relational.store.setupSuperModel( this );
		...
		findModel: function( attributes ) {
			return Relational.store.find( this, attributes );
		}
	});


	_.extend( RelationalModel.prototype, Semaphore );
